...
docker rm -f dashvanti-api dashvanti-web dashvanti-worker dashvanti-gateway dashvanti-mailpit dashvanti-migrate 2>/dev/null || true
...
docker run -d --name dashvanti-mailpit --network dashvanti-net -p 127.0.0.1:8025:8025 axllent/mailpit:v1.22
...
docker run -d --name dashvanti-worker --network dashvanti-net --env-file /home/krishna/food/.env -v dashvanti_uploads:/opt/dashvanti_fs dashvanti:latest python -m backend.worker
